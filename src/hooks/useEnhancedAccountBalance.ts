...
export const useEnhancedAccountBalance = (): EnhancedBalanceData => {
  ...
  return useMemo(() => {
    ...
    const totalUsdcBalance = usdcBalance > 0 ? usdcBalance : Math.max(fallbackBalance, 0);

    const positionsCollateralBase = openPositions
      .map((position) => {
        const marginValue =
          position.marginValueInitial.toNumber() ??
          position.initialRisk.toNumber() ??
          position.notional.times(position.adjustedImf ?? BIG_NUMBERS.ZERO).toNumber();
        if (marginValue <= 0) {
          return undefined;
        }
